...
export default async function MayorPage({ params }: { params: { city: string; mayor: string } }) {
  const mayors = await getMayors({ city: params.city });

  const mayor = mayors.candidatos.find((mayor: any) => mayor.id === parseInt(params.mayor))!;
